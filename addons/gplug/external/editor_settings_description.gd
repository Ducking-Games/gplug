...
## courtesy of https://github.com/PiCode9560/Godot-Editor-Settings-Description
## because Godot makes weird decisions about features that should be in the engine
## with flimsy excuses as to why they aren't there.
@tool
extends GDScript
...
static func _static_init() -> void:
	...
	if tree.cell_selected.is_connected(_on_editor_inspector_changed):
		tree.cell_selected.disconnect(_on_editor_inspector_changed)
	...
	if tree.cell_selected.is_connected(_on_project_inspector_changed):
		tree.cell_selected.disconnect(_on_project_inspector_changed)
